...
Procedure SetColor(const X,Y:integer; const Color,BkColor:word);
...
var
 ...
 CCI        : _Console_Cursor_Info;   // Параметры курсора
 EventsRead : DWord;                  // Количество событий, которое прочитала из буфера функция
 IBuff      : _Input_Record;          // Клавиатурное событие
...
procedure ShowCursor(const Show:boolean);
begin
If Show = False then begin; CCI.bVisible := True; CCI.dwSize := 100; end;
if Show = True then begin; CCI.bVisible := True; CCI.dwSize := 1; end;
SetConsoleCursorInfo(ConOutHandle, CCI);
end;
...
begin;
 Coord.X := X - 1; Coord.Y := Y - 1;
 FillConsoleOutputAttribute (ConOutHandle, BkColor*16+Color, 1, Coord, NOAW);
end;
...
var C:char;
    i:integer;
begin;
CCI.bVisible := True; CCI.dwSize := 1; SetConsoleCursorInfo(ConOutHandle, CCI);
...
CCI.bVisible := True; CCI.dwSize := 100; SetConsoleCursorInfo(ConOutHandle, CCI);
end;
