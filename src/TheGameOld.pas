...
begin;
putstr('Зд:'+IntToStr(h.hp.now)+'/'+IntToStr(h.hp.max),1,24,7,0);
putstr('Ма:'+IntToStr(h.mp.now)+'/'+IntToStr(h.mp.max),1,25,7,0);
putstr('Защита:('+IntToStr(h.uvert)+'/'+IntToStr(h.defen)+')['+IntToStr(h.pu)+']',15,24,7,0);
putstr(' Атака:+'+IntToStr(h.att)+'/'+IntToStr(h.dmg.a)+'d'+IntToStr(h.dmg.b)+'+'+IntToStr(h.dmg.c),15,25,7,0);
putstr('$:'+IntToStr(gold),35,24,7,0);
putstr('Время:'+IntToStr(time.h)+':'+IntToStr(time.m)+':'+IntToStr(time.s)+'   ',35,25,7,0);
end;
{==================================================}
...
var mess:string;
begin;
addecho('Сегодня '+IntToStr(time.d)+'-й день месяца '+CMonthNames[time.g]+' '+IntToStr(time.y)+'-го года',nofecho,0);
end;
{==================================================}
procedure gamecycle;
...
if allevel[px,py,2]>0 then addecho(txts[3]+' кучка '+txts[4],nofecho,0);
...
if hodi=5000 then begin;clrscr(0,0);putstr('Игра закончена',1,1,14,0);
putstr('Вы исчерпали временной лимит (8 часов 20 минут)',1,2,6,0);readok(1,3);
endall(h,gold,hodi);
hodi:=0;gold:=0;end;
